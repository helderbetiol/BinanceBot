...
function getTradersPositions() {
  ...
  for (const [id, value] of Object.entries(traders)) {
    ...
    .then(res => {
      ...
      //console.log(res.data.data.otherPositionRetList);
...
function comparePositions(trader, newPositions) {
  ...
  for (newPosition of newPositions) {
    ...
    //test
     ...
     //   console.log('chqnge opos')
     //   newPosition['updateTimeStamp'] += 1;
     //   newPosition['amount'] -= 200;
     //   // newPosition['entryPrice'] += 5;
     // }
    ...
    if (!currentPositions[newPosition['symbol']]) {
      ...
      if (newPosition['updateTimeStamp'] > currentPosition.updateTimeStamp) {
        ...
        sendUpdatePositionMsg(trader['name'], trader['link'], newPosition['symbol'], 
          variation > 0 ? buyPrice : newPosition['markPrice'], variation);
  ...
  // console.log('missing '+newPositionsSymbols)
  // console.log('missing '+Object.keys(currentPositions))
  ...
  if (missingSymbols.length > 0) {
    ...
    for (symbol of missingSymbols)
      sendDeletePositionMsg(trader['name'], trader['link'], symbol, currentPositions[symbol]['markPrice'], currentPositions[symbol]['roe']);
  }
}
